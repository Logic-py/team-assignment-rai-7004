"""Random Forest Classifier Pipeline Module.

This module implements a pipeline class for random forest classification models.
...
class RandomForestClassifierPipeline(BasePipeline):
    """Random Forest Classifier Pipeline Class."""

    def __init__(self, config: PipelineConfig) -> None:
        """Initialize the Random Forest Classifier Pipeline.

        Args:
            config: PipelineConfig, contains configuration information for a pipeline.
        """
        super().__init__(config=config)
        self.model = RandomForestClassifier(random_state=config.random_state, n_jobs=-1)
